...
Consider a program \feed which lets the user scroll through an seemingly infinite feed
of information (example due to \citeauthor{ahman2020asynchronous}). The program
cycles through its current cache of data as the user requests more; the program
simulates being infinite by making a request for another cache of data whenever
the user is nearly at the end of the current cache.
...
display data \code{d}, and then the \code{next} operations that \feed~responds
...
%
\todo{Again fix ``for some other reason''}.
%
...
\code{cont} is the continuation offered by the calling cde when the \code{await}
