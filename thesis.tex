...
Note that we can simply reuse the \code{suspend} handler from
Section~\ref{relaxing-catches}. Everything works largely the same; we run the
...
Informally, $\EC~\allows~c$ is true when $\EC$ is a handler where the ability at
the hole contains an interface which offers \yield~as a command. This could be a
handler with \yield~in its own ability, passing it down to the relevant
argument, or a handler that handles \yield~but does not have \yield~in its
ability.

\todo{Clean this up --- Maybe add examples in code?}
...
labelling the functions with \yield in the ability.
...
Figure~\ref{counting-rules} are now deterministic; for any term and counter
pair, there is only one possible reduction we can make. This is a clear
improvement on the nondeterministic semantics of Section~\ref{sec:yielding},
...
perform the nondeterministic system. We can formally state this follows.
...
One of the strengths of this approach is that these inserted \yield
...
\item[\Cse] \textsc{R-Yield-Can}
  ...
  require that
  $\checksk{\Gamma}{\sigs'_{|\many{t}|}}{A_{|\many{t}|}}{\EC'[\yield;
      n]}$. This follows from the assumption $\EC~\allows~\yield$, which
