...
\inferrule[Add-Counter]
          { \EC[n]; \bluetext{c_1, \ldots, c_n} \redtou \EC[n']; \bluetext{c_1,
              \ldots, c_n} \\
            \EF \textsf{ is handler }}
          { \EF[\EC[n]]; \bluetext{c_1, \ldots, c_n, c_{n+1}} \redtou
            \EF[\EC[n']]; \bluetext{c_1, \ldots, c_n, c_{n+1}}}
\\
\inferrule[Handle-In]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    (\bindsc{r_{k,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j} \\
  ...
  { \EC[\cu (\thunk{((r_{i,j})_j \to n_i)_i} : \thunk{\many{\effin{\adj} A
    ...
    \EC[\cu ((\many{\venv}(n_k) : B))]; \bluetext{\succc{c_1}, \ldots, \succc{c_n}} }
\\
\inferrule[Handle-In-Yield]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    (\bindsc{r_{k,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j} \\
    \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
  }
  { \EC[\cu (\thunk{((r_{i,j})_j \to n_i)_i} : \thunk{\many{\effin{\adj} A
          \to}~\effout{\sigs}B})~\many{t}]; \bluetext{c_1, \ldots, c_{n-1}, \yieldc}
    \redtou \\
    \EC[\yield!; \cu ((\many{\venv}(n_k) : B))]; \bluetext{\succc{c_1}, \ldots, \succc{c_{n-1}}} }
\end{mathpar}
\caption{New new counting}
\end{figure}
...
\inferrule[Add-Counter]
          { \EC[n]; \bluetext{c_1, \ldots, c_n} \redtou \EC[n']; \bluetext{c_1,
              \ldots, c_n} \\
            \EF \textsf{ is 1st argument to scheduler }}
          { \EF[\EC[n]]; \bluetext{c_1, \ldots, c_n, c_{n+1}} \redtou
            \EF[\EC[n']]; \bluetext{c_1, \ldots, c_n, c_{n+1}}}
\\
% \inferrule[Handle-In]
...
%     \EC[\cu ((\many{\venv}(n_k) : B))]; \bluetext{\succc{c_1}, \ldots, \succc{c_n}} }
% \\
\inferrule[Arg-Handle]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    (\bindsc{r_{k,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j} \\
    % \exists i. \adj_i = \adapt_i\pipe\ext_i \
    % \highlight{\neg (\Yield \in \ext \text{ where } \sigs = \seed\pipe\ext)}
    \highlight{\neg (\exists i. \Yield \in \ext_i \text{ for } \adj_i = \adapt_i\pipe\ext_i)  }
    % \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
  } { \EC[\cu (\thunk{((r_{i,j})_j \to n_i)_i} : \thunk{\many{\effin{\adj} A
        \to}~\effout{\sigs}B})~\many{t}]; \bluetext{c_1, \ldots,
      \justc{k}}
    \redtou \\
    \EC[\cu ((\many{\venv}(n_k) : B))]; \bluetext{\succc{c_1}, \ldots,
      \succc{\justc{k}}} }
\\
\inferrule[Arg-Yield-Can't]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    (\bindsc{r_{k,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j} \\
    % \exists i. \adj_i = \adapt_i\pipe\ext_i \
    % \highlight{\neg (\Yield \in \ext \text{ where } \sigs = \seed\pipe\ext)}
    \highlight{\neg (\exists i. \Yield \in \ext_i \text{ for } \adj_i = \adapt_i\pipe\ext_i)  } \\
    \neg (\EC \text { allows } \Yield)
    % \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
  } { \EC[\cu (\thunk{((r_{i,j})_j \to n_i)_i} : \thunk{\many{\effin{\adj} A
        \to}~\effout{\sigs}B})~\many{t}]; \bluetext{c_1, \ldots,
      \yieldc}
...
\inferrule[Arg-Yield]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    ...
    % \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
        ...
        \to}~\effout{\sigs}B})~\many{t}]; \bluetext{c_1, \ldots, c_{n-1},
      \yieldc}
    \redtou \\
    \EC[\yield!; \cu ((\many{\venv}(n_k) : B))]; \bluetext{\succc{c_1}, \ldots,
      \succc{c_{n-1}}, \justc{0}} }
\\
\inferrule[Scheduler-Exit]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    (\bindsc{r_{k,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j} \\
    % \exists i. \adj_i = \adapt_i\pipe\ext_i \
    ...
    % \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
        ...
        \to}~\effout{\sigs}B})~\many{t}]; \bluetext{c_1, \ldots, \justc{k},
      c_n}
    \redtou \\
    \EC[\cu ((\many{\venv}(n_k) : B))]; \bluetext{\succc{c_1}, \ldots,
      \succc{\justc{k}}} }
\\
\inferrule[Scheduler-Yield]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    (\bindsc{r_{k,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j} \\
    % \exists i. \adj_i = \adapt_i\pipe\ext_i \
    \highlight{\exists i. \Yield \in \ext_i \text{ for } \adj_i = \adapt_i\pipe\ext_i  } \\
    ...
    % \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
        ...
        \to}~\effout{\sigs}B})~\many{t}]; \bluetext{c_1, \ldots, \yieldc, c_n}
    \redtou \\
    \EC[\yield!; \cu ((\many{\venv}(n_k) : B))]; \bluetext{\succc{c_1}, \ldots,
      \succc{c_{n - 2}}, \justc{0}} }
\\
  \inferrule[Scheduler-Yield-Can't]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    (\bindsc{r_{k,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j} \\
    % \exists i. \adj_i = \adapt_i\pipe\ext_i \
    \highlight{\exists i. \Yield \in \ext_i \text{ for } \adj_i = \adapt_i\pipe\ext_i  } \\
...
\inferrule[Handle-Exit]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    ...
    % \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
      ...
      c_n}
    \redtou
    \cu ((\many{\venv}(n_k) : B)); \bluetext{\succc{c_1}, \ldots,
      \succc{c_n}} }
\\
\inferrule[Scheduler-Exit]
  { k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
...
\inferrule[Yield-Can]
  ...
  { \EC[m]; \bluetext{c_1, \ldots, \yield} \redtou \EC[\yield!; m];
    \bluetext{c_1, \ldots, \justc{0}}}
  \\
...
\inferrule[Yield-Can't]
  ...
  { \EC[m]; \bluetext{c_n, \ldots, c_2, \yieldc} \redtou \EC[m'];
    \bluetext{c_n, \ldots, c_2, \yieldc} }
\end{mathpar}
\caption{Concise counting, updated}
\end{figure}
...
\inferrule[Argument-Increment]
  { %k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    ...
    % \forall~j\leq~n~.~\bluetext{c_j} \not = \yieldc
    n \redtoc n'
  } { \cu (\thunk{((r_{i,j})_j \to n_i)_i; \highlight{\bluetext{c}}} : \thunk{\many{\effin{\adj} A
        \to}~\effout{\sigs}B})~(\many{t}, n, \many{n})
    \redtou \\
     \cu (\thunk{((r_{i,j})_j \to n_i)_i; \highlight{\textsf{tryReset}(\bluetext{\succc{c}}, \adj_{|\many{t}|})}} : \thunk{\many{\effin{\adj} A
        \to}~\effout{\sigs}B})~(\many{t}, \highlight{\textsf{tryYield}(n', \bluetext{\succc{c}}, \adj_{|\many{t}|})}, \many{n})
       }
\\
\end{mathpar}
\caption{Concise counting, updated}
...
\inferrule[Argument-Increment]
  { %k = \min_i\,\{i \mid \exists \many{\venv}.(\bindsc{r_{i,j}}{\effin{\adj_j} A_j}{t_j}{\sigs}{\venv_j})_{j}\} \\
    ...
    n \redtoc n'
        ...
        \to}~\effout{\sigs}B})~(\many{t}, n, \many{n})
    \redtou \\
     \cu (\thunk{((r_{i,j})_j \to n_i)_i \highlight{\bluetext{@} (\textsf{tryReset}(\bluetext{\succc{c}}, \adj_{|\many{t}|}))}} : \thunk{\many{\effin{\adj} A
        \to}~\effout{\sigs}B})~(\many{t}, \highlight{\textsf{tryYield}(n', \bluetext{\succc{c}}, \adj_{|\many{t}|})}, \many{n})
       }
\\
\end{mathpar}
\caption{Concise counting v3}
\end{figure}
