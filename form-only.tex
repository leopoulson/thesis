...
\newcommand{\counter}{c_y}
\newcommand{\justc}[1]{c({#1})}
\newcommand{\yieldc}{\textsf{yield}}
\newcommand{\plusc}{+_c}
...
\begin{mathpar}
\inferrule[R-Yield-Can]
          { \EF~\allows~\textsf{Yield} }
          { \EF[n]; \yieldc \redtou \EF[\textsf{yield!}; n]; \justc{0} }
\\
\inferrule[R-Yield-Can't]
          { \neg (\EF~\allows~\textsf{Yield}) \\
...
\item[\Cse] \textsc{R-Yield-Can} \\
  By the assumption we have that $\EF~\allows~\textsf{yield}$. This only holds
  if the evaluation frame is of the form
  \[\cu (v : \thunk{\many{\effin{\adj} A
      \to}~\effout{\sigs}B})~(\many{t},[~],\many{n'})\].

  Filling the hole then, we must have that
  \begin{mathpar}
    \EF[n] = \cu (v : \thunk{\many{\effin{\adj} A
      \to}~\effout{\sigs}B})~(\many{t},n,\many{n'}) \\
    \[\EF[\textsf{yield!}; n] = \cu (v : \thunk{\many{\effin{\adj} A
      \to}~\effout{\sigs}B})~(\many{t},\textsf{yield!}; n,\many{n'})
  \end{mathpar}

  Then assume $\checkskgs{B}{\EF[n]}$. By inversion on \textsc{T-App}, we have
  $\checksk{\Gamma}{\sigs'_{|\many{t}|}}{A_{|\many{t}|}}{n}$.

  \todo{Do we need to worry about the adaptors?}

  \todo{Fill in space here --- need to be sure that the ability permits what we
    think it does.}

  Clearly then
  $\checksk{\Gamma}{\sigs'_{|\many{t}|}}{A_{|\many{t}|}}{\textsf{yield}; n}$, as
  we know from $\EF~\allows~\textsf{yield}$ that $\yield \in
  \sigs'_{|\many{t}|}$. 
